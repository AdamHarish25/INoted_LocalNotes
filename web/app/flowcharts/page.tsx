        ...
        <div className="p-6 md:p-12 space-y-10 bg-linear-to-br from-slate-50 to-white dark:from-slate-950 dark:to-slate-900 min-h-screen">
            ...
            <div className="text-center space-y-4 mb-8">
                <div className="flex items-center justify-center gap-2 mb-2">
                    ...
                    <h1 className="text-3xl md:text-5xl font-extrabold bg-linear-to-r from-purple-600 to-blue-600 bg-clip-text text-transparent">
                        Flowcharts
                    </h1>
                </div>
                    ...
                    <CreateResourceModal type="flowchart" workspaces={workspaces || []} isGuest={isGuest}>
                        <button className="w-full h-full">
                            <Card className="h-40 border-2 border-purple-200 dark:border-purple-900/50 bg-white dark:bg-slate-900 hover:bg-purple-50 dark:hover:bg-purple-950/50 group transition-all duration-300 flex flex-col items-center justify-center hover:shadow-xl cursor-pointer hover:border-purple-400 dark:hover:border-purple-700">
                                <div className="w-14 h-14 bg-linear-to-br from-purple-500 to-purple-700 rounded-2xl group-hover:shadow-lg shadow-purple-200 dark:shadow-none flex items-center justify-center shadow-lg group-hover:scale-110 transition-all duration-300 mb-3">
                ...
                <div className="flex flex-col gap-3">
                    ...
                    <CreateResourceModal type="flowchart" workspaces={workspaces || []} isGuest={isGuest}>
                        <button className="w-full text-left">
                            <Card className="p-4 border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 border-dashed border-2 hover:border-purple-400 dark:hover:border-purple-700 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all duration-300 cursor-pointer flex items-center gap-4 group">
                                <div className="w-10 h-10 bg-linear-to-br from-purple-500 to-purple-700 rounded-lg flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform duration-300 shadow-md">
                                    <Plus className="text-white w-6 h-6" />
                                </div>
                                <span className="font-semibold text-slate-600 dark:text-slate-300 group-hover:text-purple-600 dark:group-hover:text-purple-400 transition-colors">Draw New Flowchart</span>
                            </Card>
                        </button>
                    </CreateResourceModal>
                    ...
                    {flowcharts?.map((flowchart) => (
                        <div key={flowchart.id} className="relative group/wrapper">
                            <Link href={`/flowchart/${flowchart.id}`}>
                                <Card className="p-4 hover:shadow-md hover:bg-purple-50/50 dark:hover:bg-purple-950/30 transition-all duration-300 cursor-pointer border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 flex items-start justify-between text-right gap-4 group/card hover:border-purple-300 dark:hover:border-purple-800">
                                    <div className="p-2.5 bg-purple-100 dark:bg-purple-900/40 rounded-lg text-purple-600 dark:text-purple-400 shrink-0">
                                        <Workflow className="w-5 h-5" />
                                    </div>
                                    <div className="flex-1 min-w-0 pr-8 text-left">
                                        <h3 className="text-base font-semibold text-slate-800 dark:text-slate-200 truncate group-hover/card:text-purple-700 dark:group-hover/card:text-purple-300">
                                            {flowchart.title || "Untitled"}
                                        </h3>
                                    </div>
                                    <div className="hidden sm:flex flex-col items-start justify-start gap-1 shrink-0">
                                        <span className="text-xs text-slate-400">
                                            {new Date(flowchart.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}
                                        </span>
                                    </div>
                                </Card>
                            </Link>
