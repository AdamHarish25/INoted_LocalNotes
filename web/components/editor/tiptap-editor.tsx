...
function EditorWithProvider({ provider, ydoc, noteId, initialContent, initialTitle, initialIsPublic, initialAllowPublicEditing, initialWorkspace, isReadOnly, userName, userColor }: { provider: SupabaseProvider, ydoc: Y.Doc, noteId: string, initialContent?: any, initialTitle?: string, initialIsPublic?: boolean, initialAllowPublicEditing?: boolean, initialWorkspace?: string, isReadOnly?: boolean, userName: string, userColor?: string }) {
    ...
    const handleExportPDF = async () => {
        try {
            ...
            // Fungsi pembersih untuk menghapus EMOJI dan karakter Unicode ekstrim 
            // agar tidak berubah menjadi +P pada font Helvetica bawaan jsPDF
            const sanitizeText = (str: string) => {
                if (!str) return '';
                // Menghapus emoji dan karakter di luar latin dasar
            ...
            const linesTitle = doc.splitTextToSize(sanitizeText(title || "Untitled Note"), maxWidth)
                ...
                if (node.type === 'heading') {
                    ...
                    node.content?.forEach((rowNode: any, rowIndex: number) => {
                        ...
                        rowNode.content?.forEach((cellNode: any) => {
                            ...
                            rowData.push(sanitizeText(text))
                        })
        ...
        <div className="flex flex-col h-full bg-slate-50 dark:bg-background">
            ...
            <div className="flex items-center justify-between px-2 md:px-6 py-2 md:py-3 border-b border-gray-100 dark:border-b-gray-500 mb-2 md:mb-4 sticky top-0 bg-white dark:bg-card z-40 transition-colors print-hidden shadow-sm">
                <div className="flex items-center gap-1 md:gap-3 flex-1 min-w-0">
                    <button onClick={() => router.back()} className="p-1.5 md:p-2 border dark:border-border rounded-md bg-slate-50 dark:bg-muted hover:bg-slate-100 dark:hover:bg-muted/80 dark:text-white transition-colors shrink-0">
